...
echo      [https://github.com/insovs/EnablePowerShellScript]
echo.
...
echo     so that you can run PowerShell scripts without interruption.
echo.
...
:enable
cls
echo  [*] Unblocking PowerSwhell Execution Script...
...
echo  [+] Done. Restart necessary to apply full changes.
goto restart

:revert
cls
echo  [*] Reverting Execution Policy...
reg delete "HKCR\Applications\powershell.exe\shell\open\command" /ve /f >nul 2>&1
...
echo  [+] Reverted. Restart necessary to apply full changes.
goto restart
...
echo Restart necessary to apply full changes.
